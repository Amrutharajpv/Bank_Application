...
public class UserServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		//	adminservice.depositeMoney(us);
			...
			message="Amount Credited Sucessfully.....!!!";
			session.setAttribute("message",message);
			response.sendRedirect("view.jsp");
			...
			if(c==true)
			{
				...
				if(c1==true)
				{
					try {
						...
						message=" Transfered Sucessfully.....!!!";
						session.setAttribute("message",message);
						response.sendRedirect("view.jsp");
			...
			if(c==true)
			{
				...
				if(b2==true)
				{
					try {
						...
						message="Transaction is Sucessfully Amount Withdrawn.....!!!";
						session.setAttribute("message",message);
						response.sendRedirect("view.jsp");
					}
					catch(IOException e) {
				...
				else
				{
					try {
						PrintWriter pwt=response.getWriter();
						pwt.println("check the password");
					}
					catch(IOException e) {
						e.printStackTrace();
					}
		...
		else if(action.equals("passwordchange"))
		{
			...
			{
					response.getWriter().println("Password Is Incorrect..!!Please check and Re-Enter The Password");			
					}
			}
